...
namespace Reto5
{
  ...
  public class MainActivity : Activity
  {
    ...
    const string emailParticipante = "cob3x85@outlook.com";
    ...
    {
      var path = Path.Combine(System.Environment.GetFolderPath(System.Environment.SpecialFolder.Personal), localDbFilename);
      if (!File.Exists(path))
      {
    ...
    private async Task SyncAsync()
    {
      try
      {
        await torneoItemTable.PullAsync("allTorneoItems", torneoItemTable.CreateQuery().Where(item => item.Email == emailParticipante));
      }
      catch (Exception e)
      {
        var error = e.Message;
